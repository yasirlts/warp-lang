# Warp Commerce Type Specification v0.2
...
**Supersedes:** [v0.1](WARP_TYPE_SPEC_v0.1.md). v0.2 keeps every v0.1
type intact and adds four normative sections — serialization
conventions, the PhoneNumber adapter normalization contract, nullable
field semantics, and the offer-branch invariant — plus three new
types (`Occasion`, `SegmentCriteria`, `ABTestVariant`). The full
delta is in the [Changelog](#changelog).
...
abandoned-cart, post-purchase, and re-engagement workflows; the
`Occasion` enum is what selects them.

**Variants.** `Ramadan | Eid | BlackFriday | NewYear | BackToSchool |
Mothers Day | None`.

**Notes.**

- MENA-first defaults: `Ramadan` and `Eid` are first-class variants,
  not generic "holiday" tags. The template library carries calibrated
  templates per occasion (P-7).
- `None` is a valid variant — workflows that don't care about
  occasion-tagging carry it explicitly so absence-of-tag is
  observable.

**Display format.** PascalCase variant names in logs.

**Serialization format (JSON).** Lowercase: `"ramadan" | "eid" |
"black_friday" | "new_year" | "back_to_school" | "mothers_day" |
"none"`.

---

### SegmentCriteria *(new in v0.2)*

**Definition.** The typed predicate `CustomerSegment` evaluates over
a list of `CustomerProfile`s to produce a `CampaignAudience`.

**Fields.**

| Field                | Type                  | Notes                                           |
|----------------------|-----------------------|-------------------------------------------------|
| `min_purchases`      | `Option<u32>`         | exclude customers with fewer than N orders      |
| `min_total_spend`    | `Option<Currency>`    | exclude below this lifetime spend               |
| `languages`          | `Vec<Language>`       | empty = no language filter                      |
| `preferred_channels` | `Vec<Channel>`        | empty = no channel filter                       |
| `occasion`           | `Option<Occasion>`    | restrict to a specific marketing occasion       |

**Invariants.**

- `min_total_spend` is `Option<Currency>`, not `Option<Decimal>` —
  spend filters must declare a currency. A filter set in MAD applied
  to a customer base reported in EUR is a compile-time error from the
  predicate engine, not a runtime semantic bug.
- An empty `SegmentCriteria` (all fields `None` / empty) matches every
  customer. The composition is AND across fields; OR-composition uses
  multiple `CustomerSegment` nodes piped into a `CampaignAudience`
  union.
...
{
  "min_purchases": 2,
  "min_total_spend": { "amount": "500.00", "code": "MAD" },
...
### ABTestVariant *(new in v0.2)*

**Definition.** The label that `ABTestRoute` assigns to a workflow
invocation so downstream nodes can branch on the assignment.

**Variants.** `A | B | C | D` (four-way max in v0.2; opens the door
to wider tests without immediately requiring them).

**Invariants.**

- Variant assignment is sticky per `(experiment_id, customer_id)` —
  the same customer in the same experiment always gets the same
  variant. The assignment store lives in `warp-storage`; the type
  carries only the label.
- The set is closed; adding `E` requires a Warp release.

**Display format.** Single uppercase letter (`"A"`, `"B"`, `"C"`,
`"D"`).
...
- **Minor** bumps for additive changes — new optional fields on
  ...
  types. v0.2 is a minor bump from v0.1 (three new types, four new
  normative sections, no breaking change to any v0.1 type).
...
- `CampaignAudience` — `List<CustomerProfile>` with segment metadata.
  v0.2 ships `SegmentCriteria` (the predicate); the audience type
  that the predicate produces lands with the first node that emits
  it.
