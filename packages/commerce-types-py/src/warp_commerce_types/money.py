...
def _money_breakdown_rule() -> dict:
    """Locate the canonical ``money_breakdown_sum`` rule expression inside
    schema/behavior/invariants.json (it lives under invariant I-1's rule
    expressions). Returns the expression node so the validator is provably wired
    to the canonical source."""
    for inv in _invariants()["invariants"]:
        for expr in inv.get("rule", {}).get("expressions", []):
            if expr.get("id") == "money_breakdown_sum":
                return expr
    raise KeyError("money_breakdown_sum rule not found in canonical invariants.json")


# Loaded once; presence asserts the rule exists in the canonical schema.
_BREAKDOWN_RULE = _money_breakdown_rule()


def validate_money_breakdown(breakdown: "MoneyBreakdown") -> None:
    """Enforce the canonical ``money_breakdown_sum`` rule (schema/behavior/invariants.json,
    invariant I-1):

        all(c.amount.currency == total.currency for c in components)
        AND money_equals(sum(c.amount.amount for c in components), total.amount, total.currency)

    i.e. every component shares the total's currency and the component amounts sum
    to the total within the currency's minor-unit tolerance (Discount components
    carry a negative amount and therefore subtract). Raises on violation."""
