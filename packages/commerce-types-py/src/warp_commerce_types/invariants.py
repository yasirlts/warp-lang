...
violation. The invariant names and fix hints are read from
``schema/behavior/invariants.json`` (the same source the TS binding uses).
"""
...
def _violation(inv_id: str, description: str) -> InvariantViolation:
    ...
    return InvariantViolation(
        invariant=inv_id, name=meta["name"], description=description, fix=meta["fix"]
    )
