...
_COMMITMENT_EDGES: Dict[str, List[str]] = _TABLES["commitment"]["edges"]
_INTENT_EDGES: Dict[str, List[str]] = _TABLES["intent"]["edges"]
_FULFILLMENT_EDGES: Dict[str, List[str]] = _TABLES["fulfillment"]["edges"]
_FULFILLMENT_SPECIAL: List[dict] = _TABLES["fulfillment"].get("special_cases", [])
...
def is_valid_fulfillment_transition(from_state: Any, to_state: Any) -> bool:
    ...
    # Payload-gated special cases (Failed -> Planned only when recoverable).
    for case in _FULFILLMENT_SPECIAL:
        if case["from"] == from_type and case["to"] == to_type:
            return bool(_field_of(from_state, case["condition_field"]))
    # A Failed state with no matching special case never advances by the table.
    if from_type == "Failed":
        return to_type in _FULFILLMENT_EDGES.get("Failed", [])
